...
def draw_mel_spectrograms(dataset: RecordingDataset, show: bool = True):
    """
    ...
    fig, axes = plt.subplots(
        n_rows, n_cols, figsize=(15, 3 * n_rows), constrained_layout=True
    )
    ...
    # Save the figure to a file for later analysis
    plt.savefig("mel_spectrogram_samples.png", dpi=200)
    if show:
        plt.show()
    plt.close(fig)


def save_mel_stats_tables(
    dataset: RecordingDataset,
    csv_path: str = "mel_spectrogram_stats.csv",
    md_path: str = "mel_spectrogram_stats.md",
):
    """
    Saves basic stats per training recording to CSV and Markdown.
    Useful for analysis within/across speakers.
    """
    rows = []
    for person in dataset.training_set:
        for digit in RECORD_NAMES:
            try:
                rec = person.get_recording(digit)
            except KeyError:
                continue
            duration_sec = len(rec.raw_recording) / rec.sample_rate
            mean_mel_db = float(np.mean(rec.mel_db))
            std_mel_db = float(np.std(rec.mel_db))
            centroid = librosa.feature.spectral_centroid(
                y=rec.agc_normalized_recording, sr=rec.sample_rate
            )
    ...
    with open(md_path, "w") as f:
        f.write("# Mel Spectrogram Stats (Training Set)\n\n")
        f.write("| " + " | ".join(header) + " |\n")
...
def calc_dtw_table(reference: PersonRecordings, training_set: List[PersonRecordings]):
    s = time.time()
    dtw_table = np.full(
        (len(training_set), len(reference.recordings), len(reference.recordings)),
        -1,
        dtype=np.float32,
    )
    for train_recorder_index, person in enumerate(training_set):
        for train_record_number, train_record_name in enumerate(RECORD_NAMES):
            current_train_record = training_set[train_recorder_index].get_recording(
                train_record_name
            )
            for reference_recond_number, reference_record_name in enumerate(
                RECORD_NAMES_WITH_RANDOM
            ):
                current_reference_record = reference.get_recording(
                    reference_record_name
                )
                current_dtw = calc_dtw(
                    current_train_record.mel_db.T, current_reference_record.mel_db.T
                )
                normalized_current_dtw = current_dtw / (
                    current_reference_record.mel_db.shape[1]
                    + current_train_record.mel_db.shape[1]
                )  # normalize by length
                dtw_table[train_recorder_index][train_record_number][
                    reference_recond_number
                ] = normalized_current_dtw
            print(
                f"finish {str(train_record_number) + 'th' if train_record_number < 10 else 'random'} record of"
                f" {training_set[train_recorder_index].recoder_name}"
            )
        print(
            f"finish comparing {training_set[train_recorder_index].recoder_name} records"
        )
...
def load_dataset(path: str) -> RecordingDataset:
    ...
    reference_dirs = [d for d in reference_dir.iterdir() if d.is_dir()]
    ...
    if not reference:
        raise ValueError(
            "Reference directory must contain at least one recorder directory"
        )
...
def update_cost_in_current_indices(
    accumulated_cost_matrix: NDArray[np.float32],
    dist_matrix,
    index_record_b: int,
    index_record_a: int,
):
    current_cell_value = dist_matrix[index_record_b, index_record_a]
    accumulated_cost_matrix[index_record_b, index_record_a] = current_cell_value + min(
        cell_value_or_inf(accumulated_cost_matrix, index_record_b - 1, index_record_a),
        cell_value_or_inf(accumulated_cost_matrix, index_record_b, index_record_a - 1),
        cell_value_or_inf(
...
def calc_dtw_c(record_a: NDArray[np.float32], record_b: [np.float32], dist_matrix):
    ...
    for index_record_b in range(1, len(record_b) + 1):
        for index_record_a in range(1, len(record_a) + 1):
            accumulated_cost_matrix[index_record_b, index_record_a] = dist_matrix[
                index_record_b - 1, index_record_a - 1
            ] + min(
                accumulated_cost_matrix[index_record_b - 1, index_record_a],
...
def plot_dtw_heatmap(training_set, dtw_table):
    ...
    for s in range(n_speakers):
        for d in range(n_digits):
            y_labels.append(
                f"{training_set[s].recoder_name} - {str(d) if d < 10 else 'random'}"
            )
...
def evalute_threshold(threshold_to_check: float, dtw_table: NDArray[np.float32]):
    ...
    good_classifications_with_threshold = (
        min_dtw_per_recorder_and_number <= threshold_to_check
    ) & classifications
    bad_classification_with_threshold = (
        min_dtw_per_recorder_and_number <= threshold_to_check
    ) & (~classifications)
    return np.sum(good_classifications_with_threshold) - np.sum(
        bad_classification_with_threshold
    )
    ...
    for recorder_index in range(predict.shape[0]):
        for number in range(predict.shape[1]):
            if min_dtw[recorder_index, number] < threshold_to_check:
                predictions_with_threshold[recorder_index, number] = (
                    str(predict[recorder_index, number])
                    if predict[recorder_index, number] < 10
                    else RANDOM_RECORD_NAME
                )
...
def accuracy_of_threshold(threshold_to_check: float, dtw_table: NDArray[np.float32]):
    ...
    correct_numbers_predictions = np.sum(
        (numbers_predictions.ravel() == true_numbers)
        & (min_dtw_numbers.ravel() < threshold_to_check)
    )

    min_dtw_per_random_recording = min_dtw[:, 10]
    correct_random_predictions = np.sum(
        min_dtw_per_random_recording < threshold_to_check
    )

    return (correct_numbers_predictions + correct_random_predictions) / (
        dtw_table.shape[0] * dtw_table.shape[1]
    )
...
def evaluate_classifications(dtw_table: NDArray[np.float32]):
    ...
    for recorder_index in range(argmin_dtw_per_recorder_and_number.shape[0]):
        for number in range(argmin_dtw_per_recorder_and_number.shape[1]):
            classifications[recorder_index, number] = (
                argmin_dtw_per_recorder_and_number[recorder_index, number] == number
            )
...
def run_force_align_pickle(path: str):
    ...
    plot_alignment_on_probs(
        probs, ext, path_s, id2label, "CTC forced alignment (pickle)"
    )
    plt.figure(figsize=(6, 4))
    plt.imshow(back.T, aspect="auto", origin="lower", cmap="viridis")
    plt.colorbar(label="backtrace src state")
...
def main():
    ...
    train_dtw_table = np.delete(
        training_set_dtw_table, 10, axis=1
    )  # this is the array we learn with.
    ...
    print("train score by our measure: ", train_score)
    validation_score = accuracy_of_threshold(best_threshold, validation_set_dtw_table)
    print("validation score by our measure: ", validation_score)
    ...
    predict_validation = predict_with_threshold(
        best_threshold, validation_set_dtw_table
    )
    y_pred_train = predict_train.ravel()
    labels = [str(i) for i in range(0, 10)] + ["random"] + ["AT"]
    y_true_train = np.tile(
        [str(i) for i in range(0, 10)] + ["random"], len(dataset.training_set)
    )

    y_pred_validation = predict_validation.ravel()
    y_true_validation = np.tile(
        [str(i) for i in range(0, 10)] + ["random"], len(dataset.validation_set)
    )
    ...
    plot_confusion_matrix(
        y_true_train,
        y_pred_train,
        [str(i) for i in range(0, 10)] + ["random"],
        labels,
        "training",
    )
    plot_confusion_matrix(
